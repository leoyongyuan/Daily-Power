...
在这里我们补上then中最后一块内容，就是判断传入的参数是否为函数，如果不为函数则为它转变成一个返回传入值的函数即可。
```Javascript
then(onResovled,onRejected) {
...
```
这样值穿透就可以被实现，在我们传入非函数时，就返回上一个参数的值，同理我们也把异常穿透也按同样的方法实现，如果发生报错，就一直把错误往下抛。

catch捕获异常穿透：catch的实现其实非常容易，catch的作用是来捕获错误的，其它大致内容和then方法一致，所以只需要传入一个reject函数即可。
```
catch(onRejected) {
